...
- Transform一覧をプレハブ1つにつき1カラムに変更。入れ子・同名の別インスタンスを別列にし、非プレハブは専用列へ集約。
- プレハブ名の見出しを追加。各列のTransformを独立してページ切替でき、画面幅に収まらないプレハブ列も専用ボタンで切替可能。
- 近傍パネル・画面端の両配置に対応。列の切替前後でパネルサイズを維持し、Transformのドラッグ操作を継続。

...
`D9speed > Animation > Animator Playback Preview`から開きます。対象Animatorと検索フォルダを指定し、Play ModeでController・State・AnimationClipを選んで再生します。ランダム再生や手指のマスクに対応します。

## Random Hand Muscle Generator

`D9speed > Animation > Random Hand Muscle Generator`から開き、有効なHumanoid Avatarを持つAnimatorを指定します。左右の手、指・親指・開きの範囲、変化の間隔を設定し、1回適用または定期的なランダム化を行えます。編集モードとPlay Modeに対応します。

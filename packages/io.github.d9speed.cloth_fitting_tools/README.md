...
VRChatアバター向けのPhysBoneコライダー作成と、ProxyボーンへのConstraint設定をまとめたEditor専用パッケージです。
...
## ProxVRCphysboneSettings

`D9speed > Tools > ProxVRCphysboneSettings`を開き、ActualチェーンとProxy Targetを指定します。距離に応じて複数のActualボーンを対応付け、VRC Rotation ConstraintまたはVRC Parent Constraintを設定します。プレビューを確認して適用してください。

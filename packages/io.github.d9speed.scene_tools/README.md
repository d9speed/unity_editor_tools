...
- カラム内のTransformアイコン付き名前ラベルを、インスペクターのTransform参照欄へ直接ドラッグできます。ドラッグ中は選択を維持し、クリックで対象を選択します。
- 「プレハブごとにカラム配置」で、1つのプレハブインスタンスを1列にまとめます。衣装・髪など入れ子のプレハブや同名の別インスタンスも別列にし、所属しないTransformは「非プレハブ」列に表示します。
- 画面幅に収まる数の列を横に並べ、残りは上部の「プレハブ列」の < > で切り替えます。Transformのページは各列で独立し、プレハブ名の見出しはページをまたいでも表示します。近傍パネル・画面端の両配置に対応します。
